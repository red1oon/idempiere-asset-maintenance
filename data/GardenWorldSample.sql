...
-- 10. WORK ORDERS
-- ============================================
-- c_doctype_id = 50010 (Maintenance Order, docbasetype='MOF')
-- IMPORTANT: C_DocType_ID is REQUIRED for forms to show records!
INSERT INTO mp_ot (mp_ot_id, ad_client_id, ad_org_id, created, createdby, updated, updatedby,
    isactive, documentno, a_asset_id, mp_maintain_id, mp_jobstandar_id, c_doctype_id,
    datetrx, description, docstatus, docaction, processed, mp_ot_uu)
VALUES
-- Draft - shows in Programming OT form (DocStatus='DR', tasks all 'NS')
(1100001, 11, 11, NOW(), 101, NOW(), 101, 'Y',
    'WO-001', 1100001, 1100001, 1100001, 50010,
    NOW(), 'Oil change for Truck #1', 'DR', 'CO', 'N', uuid_generate_v4()),
-- In Progress - work started
(1100002, 11, 11, NOW(), 101, NOW(), 101, 'Y',
    'WO-002', 1100003, 1100003, 1100003, 50010,
    NOW() - INTERVAL '1 day', 'Battery service Forklift #1', 'IP', 'CO', 'N', uuid_generate_v4()),
-- Completed
(1100003, 11, 11, NOW(), 101, NOW(), 101, 'Y',
    'WO-003', 1100002, 1100002, 1100002, 50010,
...
UNION ALL SELECT 'Prognosis', count(*) FROM mp_prognosis WHERE ad_client_id=11 AND mp_prognosis_id >= 1100001
...
-- ============================================
-- 13. PROGNOSIS (Forecast) RECORDS
-- ============================================
-- Prognosis records are PROCESS-GENERATED only.
-- AD_PInstance_ID is mandatory - records created by "Maintenance Forecast" process.
-- Run: Menu > Asset Maintenance > Maintenance Forecast
-- No manual INSERT possible due to AD_PInstance_ID NOT NULL constraint.

-- ============================================
-- FORMS CHECKLIST - What each form needs:
-- ============================================
--
-- 1. GENERATE WORK ORDER (WMPGenerateOT)
--    Query: MP_Prognosis WHERE Processed='N' AND Selected='N'
--    INNER JOIN: MP_Maintain, A_Asset
--    Sample: 3 prognosis records above ✓
--
-- 2. SUBMIT MAINTENANCE REQUEST (WMPRequestOT)
--    Query: MP_OT_Request WHERE DocStatus='WC' AND Processed='Y'
--    INNER JOIN: AD_User, A_Asset, AD_Ref_List(154)
--    LEFT JOIN: MP_JobStandar
--    Sample: REQ-001 has DocStatus='WC' ✓
--
-- 3. PROGRAMMING OT / OT LIST (WMPProgrammingOT)
--    Query: MP_OT WHERE DocStatus='DR'
--           AND NOT EXISTS (tasks with STATUS<>'NS')
--    INNER JOIN: A_Asset, C_DocType
--    LEFT JOIN: AD_User
--    Sample: WO-001 has DocStatus='DR', C_DocType_ID=50010, tasks='NS' ✓
--    NOTE: This form is NOT in the standard menu!
-- ============================================
